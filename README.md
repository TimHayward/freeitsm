...
> **Status (chunks A + B + C)**: working editor end-to-end. Drag a class tile onto the canvas → pick which CMDB object to bind it to → node is placed (snapped to 20px grid), draggable to move, Delete to remove. Planned objects render with dashed border + amber tint matching the CMDB browse styling. Autosave, manual save, save-as-new-version, read-only handling for historical versions all in place. **Connectors + relationships pull-in land in chunk D.**
